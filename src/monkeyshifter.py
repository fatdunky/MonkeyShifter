...
def setup_configuration(config_directory,config_file):
    ...
    Setup configuration
    '''
    return Configuration(config_directory,config_file)

def setup_logging(console_mode,log_file,log_directory, log_level):
    '''
    Setup logging
    '''
    Logger.setLogDirectory(log_directory)
    Logger.setLogFile(log_file)
    Logger.setLoggingLevel(log_level)
    Logger.setRotationFile(True, 'midnight', 1, 0)
    Logger.updateHandler()
...
def load_word_files(config, word_files):   
    ...
    for word_file_name in list(config.get_word_file_names().values()):
        ...
        word_class = Io.load_class(class_string)
...
def load_media_type_class(config, media_type_name,media_types):
    ...
    class_string = "lines.mediatypes." + config.get_media_type_module_name(media_type_name) + "." + config.get_media_type_module_class(media_type_name)
    logging.debug("Loading class : %s",class_string)
    media_type_class = Io.load_class(class_string)
...
def load_media_sub_type_class(config, media_sub_type_name,media_type_name,media_types):
    ...
    class_string = "lines.mediatypes." + config.get_media_sub_type_module_name(media_type_name,media_sub_type_name) + "." + config.get_media_sub_type_module_class(media_type_name,media_sub_type_name)
...
def parse_file_names(file_names):
    ...
    for line in names_to_parse:
        ...
        if (len(current_unmatched_word_list) > 0):
            unmatched_words_map[line] = current_unmatched_word_list
...
def main(argv):
    ...
    setup_logging(console_mode,log_file,log_directory, log_level)
    ...
    if (test_mode and test_file != ""):
        ...
        names_to_parse = Io.read_file(test_file)
